...
public class BinaryTree {
	...
	public int getSuccessor(TNode n) {
		...
		if (n.leftChild == null && n.rightChild == null) {
			...
			TNode nParent = n.parent;
			if (n.parent.rightChild == n || nParent == null) {
				while (nParent.data <= n.data) {
			...
			result = getMinimumFromBST(n.rightChild);
			return result.data;
		}
	...
	public List getTrail(TNode node) {
	...
	public int getFirstCommonAncestor(TNode node1, TNode node2) {
		int size, offset;

		List t1 = getTrail(node1);
		List t2 = getTrail(node2);
		...
		if (t1.size() >= t2.size()) {
			size = t1.size();
			offset = t1.size() - t2.size();
			base = t1;
			test = t2;
		} else {
			size = t2.size();
			offset = t2.size() - t1.size();
			base = t2;
			test = t1;
		}

		for (int i = 0; i < size-1; i++) {
			if (base.get(offset + i) == test.get(i)) {
				TNode result = (TNode) base.get(offset + i);
				return result.data;
	...
	public int getRandomNode()
	{
		...
		List l = inOrderArray(root);
		int value = rand.nextInt(l.size());
		// int value = rand.nextInt(50);
		// gives random number between 0 to 49
	...
	public static void main(String[] args) {
		...
		//System.out.println(bt.getFirstCommonAncestor(bt.root.leftChild.leftChild, bt.root.leftChild.leftChild.rightChild));
		...
		//System.out.println(bt.root.leftChild.rightChild.data);
		//System.out.println(bt.getSuccessor(bt.root.leftChild.rightChild));

		// System.out.println("Is it a subtree ? :" + bt.isSubTree(bt.root,
		// bt1.root));
